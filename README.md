...
- **主席控制台 `/chair?id=`**：開會時的即時操作——設現行議程、對表決案「開始表決／停止並宣佈結果」（需求：會議主席控制台）。**會議一開始的現行階段是「簽到」**（`current_agenda_item_id = NULL`），投屏顯示全員名單（未到排前面、可捲動）＋實到／應到；主席按「下一案」才進議程 1，從議程 1 按「上一案」回到簽到。
- **請假**：不另設請假登記。「出席 X」的分母是應到人數（名單全體），請假＝未簽到＝留在分母，這是法定人數的正確算法（扣分母等於大家請假就能降門檻）；真的不該在名單上的人在工作台 ② 移除。
- **分頁標題**：每頁 `<title>` 是「主題 · 會議名｜T-Meeting」（投放畫面／主席控制台／簽到台／工作台／投票紀錄／表決…），管理者同時開多個分頁分得出來。
...
- **通過判定**（停止表決時寫入快照）：門檻只有四種——**相對多數**（同意 > 不同意）、**1/2**（同意 > 已簽到 ÷ 2）、**2/3**、**3/4**（同意 ÷ 已簽到 ≥ 比例）。分母一律已簽到人數（未投視同不同意），不看出席法定人數，因此**未簽到不能投票**。**同票**（相對多數平手、1/2 剛好一半）畫面只標「同票」不寫通過／不通過，由主席裁示。規則在 `lib/threshold.ts`，有測試；舊值 `1/2+1/2`／`2/3+1/2`→`1/2`、`2/3+2/3`→`2/3` 由 `initDb` 自動換算。
- **結束**：主席在最後一案按「結束會議」（主席控制台或投屏控制列），投屏變成「會議已結束」，其他頁面正在看的人跳出提示；工作台 ① 可重新開啟。
