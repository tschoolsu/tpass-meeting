        ...
        {participants.map((p) => (
          <li key={p.email} className="flex items-center justify-between gap-3 py-2">
            <span className="min-w-0 truncate text-sm font-bold" title={p.email}>
              {displayName(p)}
              {p.name ? <span className="ml-2 font-mono text-xs font-medium text-muted-foreground">{p.email}</span> : null}
              {p.grade ? <span className="ml-2 font-mono text-xs text-muted-foreground">[{p.grade}]</span> : null}
            </span>
            <span className="flex shrink-0 items-center gap-2">
              ...
              <ConfirmActionButton
                ...
                confirm={{
                  title: `確定要把 ${displayName(p)} 移出名單嗎？`,
