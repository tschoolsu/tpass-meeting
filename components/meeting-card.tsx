...
// 整張卡可點（標題的 Link 用 after: 偽元素撐滿整張卡），刪除鈕疊在上面（relative z-10），
// 所以 button 不會巢在 a 裡。canDelete 只決定要不要畫按鈕，權限仍在 action 內檢查。
export function MeetingCard({ meeting, canDelete = false }: { meeting: MeetingListItem; canDelete?: boolean }) {
  const phase = derivePhase(meeting.status, meeting.starts_at);
  return (
    <Card className="group relative flex h-full flex-col items-start gap-3 text-left transition-all duration-200 hover:-translate-y-1 hover:shadow-[7px_7px_0_0_var(--color-foreground)] active:translate-y-0 active:shadow-[3px_3px_0_0_var(--color-foreground)]">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="font-mono text-xs font-bold text-muted-foreground">{formatTaipei(meeting.starts_at)}</span>
      </div>

      <h2 className="text-lg font-extrabold leading-snug">
        <Link href={`/read?id=${meeting.id}`} className="after:absolute after:inset-0 after:content-['']">
      ...
      {canDelete ? (
        <span className="relative z-10 self-end">
          <DeleteMeetingButton meetingId={meeting.id} title={meeting.title} size="sm" variant="ghost" afterDelete="stay" />
        </span>
      ) : null}
    </Card>
  );
